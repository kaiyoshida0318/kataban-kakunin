...
- 「商品」列の件数ボタンで行を展開すると、関連URLを表で管理できる（追加日 / 画像 / 商品名 / 商品URL / 編集 / 確認 / 買付 / 操作）
- 商品行の右には状態のドロップダウンが2つ。**確認**（確認前 / 確認後 / スキップ）と **買付**（買付前 / 買付済 / スキップ）。選んだ瞬間に保存される
- 商品テーブルの1行目は**常に空の入力行**。そこに直接入力してEnter（または「追加」）で積み上がる。追加後はまた空行に戻り、カーソルもURL欄に戻るので連続投入できる
- 「商品追加」列の **＋ 商品** を押すと、その行が開いて入力行のURL欄にカーソルが入る
- 商品テーブル右上の **× 商品欄を閉じる** で、その行の商品欄をたたむ
