...
最終更新: 2026-08-26 / 現行版 **v0.93.0**

次のチャットに渡すもの:

1. `katabankakuninv0.93.0full.zip`（全ファイル入り。**これが唯一の正**）
...
unzip -oq /mnt/user-data/uploads/katabankakuninv0.93.0full.zip -d /root/work/katabank
...
- テストスクリプト: `/root/work/t*.js`（52本。`katabank-tests.zip` で渡している）
  - よく回すもの: **t53**(列管理の並び/非表示) **t54**(列を隠した状態での追加・保存・リロード)
    ...
    **t89**(赤系で行が赤くなる) **t90**(出所2行) **t91**(操作列に編集ボタン) **t92**(ジャンル名の改行) **t93**(件表示ボタンの色) **t94**(出所のジャンル名を全部出す) **t95**(選び直した直後の行の印)
...
- **赤系（red / pink）の選択肢が1つでも付いた商品は行ごと赤くする**（v0.85.0）。判定は `pickAlert(p)`、
  クラスは `tr.pk-ng`。追加した商品・チェックした商品の両方に付ける。
- **ドロップダウンで選び直した直後は表を描き直さない。** 行に付く印は
  `paintPickRow(row, p)` が今の値から**全部まとめて**付け直す（v0.93.0）:
  左の帯 `bar-*`（`pickBarColor`）/ 薄くする `wk-done`（`pickDone`）/ 赤 `pk-ng`（`pickAlert`）。
...
**「N 件表示」ボタンは中の商品が赤いかどうかで塗り分ける（v0.90.0）。** 判定は行の色（`pk-ng`）と
**同じ `pickAlert()`**。別々に書くと、ボタンと開いたときの行の色が食い違う。

- `cntMix(picks)` が `CNT_TONES`（白 → 赤）の順で種類を1回ずつ返し、`cntBg()` が等分の
  `linear-gradient(90deg, …)` にする。**件数比ではなく等分**（ユーザー指定。2種類=50%ずつ）。
  件数は `title` の内訳にだけ出す（1種類だけのときは出さない）。
- **0件は白、全部ふつう（白だけ）なら塗らない（既定の青）**（v0.92.0でユーザーが指定）。
  「商品が入っていない＝白 / 入っていてふつう＝青 / 赤が混ざる＝赤の帯」の3段階。
  ここを塗ると、赤が1件も無い普通の行まで全部同じ色になって「変わっていない／おかしい」と言われる（v0.89.0で実際に言われた）。
...
- `pickBarColor(p)`（「隙あり/なし」の色）は**別もの**。「追加した商品」の行の左端の帯（`bar-*`）用。
...
- **v0.93.0 ドロップダウンで選び直した直後に行の印（薄く・帯・赤）が付け替わらない不具合を修正（最新）**
