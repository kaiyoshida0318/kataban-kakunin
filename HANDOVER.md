...
- 対象は `genreAllTargets()`。**`visibleItems()`（＝検索・絞り込み後の行）× そのタブの `urlFields`** を回して、
  `GENRE_FIELDS`（`urlRakuten→name` / `urlAmazon→nameAmazon`）が**空のものだけ**返す。
  文字モードの欄（`isTextMode`）は URL ではないので外す。**入っている名前は絶対に上書きしない**
  （書く直前にも `write()` でもう一度空か見る。走っている間に手で入れられることがある）。
- **同時2本の worker（`Promise.all([worker(), worker()])`）＋1件ごとに 400ms 待つ。**
  中継は連打すると 429 を返すので、ここを上げると全滅する。**6連敗（`streak >= 6`）で自動打ち切り。**
- **止める**のはボタン再押下 → `genreAllStop = true`。走っている間はボタンが赤い進捗表示
  （`paintGenreAllBtn()`）。`renderToolbar()` は**実行中は文言を書き換えない**（`if (!genreAllBusy)`）。
- 保存は最後に1回（`persistLocal()` + `markDirty(true)` + `renderAll()`）。1件ずつ `upsert()` すると
  そのたび全描画が走って重い。
- 出すのは `autoGenreTab(view)` のタブだけ（楽天ライバルは店名なので対象外）。
...
（`.src-name`）。折り返すので `.added-tbl .td-src{white-space:normal}`、ジャンル名は
`.src-name{-webkit-line-clamp:2}` で2段まで。**揃え設定は `.src-name` にも当てること**
