...
最終更新: 2026-08-26 / 現行版 **v0.92.0**

次のチャットに渡すもの:

1. `katabankakuninv0.92.0full.zip`（全ファイル入り。**これが唯一の正**）
...
unzip -oq /mnt/user-data/uploads/katabankakuninv0.92.0full.zip -d /root/work/katabank
...
- テストスクリプト: `/root/work/t*.js`（51本。`katabank-tests.zip` で渡している）
  - よく回すもの: **t53**(列管理の並び/非表示) **t54**(列を隠した状態での追加・保存・リロード)
    **t56**(列管理モーダル) **t59**(チェックした商品の列) **t85/t86**(項目管理・項目の増減)
    **t89**(赤系で行が赤くなる) **t90**(出所2行) **t91**(操作列に編集ボタン) **t92**(ジャンル名の改行) **t93**(件表示ボタンの色) **t94**(出所のジャンル名を全部出す)
