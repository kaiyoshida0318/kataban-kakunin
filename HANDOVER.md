...
最終更新: 2026-08-26 / 現行版 **v0.94.0**

次のチャットに渡すもの:

1. `katabankakuninv0.94.0full.zip`（全ファイル入り。**これが唯一の正**）
...
unzip -oq /mnt/user-data/uploads/katabankakuninv0.94.0full.zip -d /root/work/katabank
...
- テストスクリプト: `/root/work/t*.js`（52本。`katabank-tests.zip` で渡している）
  - よく回すもの: **t53**(列管理の並び/非表示) **t54**(列を隠した状態での追加・保存・リロード)
    **t56**(列管理モーダル) **t59**(チェックした商品の列) **t85/t86**(項目管理・項目の増減)
    **t89**(買付で行の色が決まる) **t90**(出所2行) **t91**(操作列に編集ボタン) **t92**(ジャンル名の改行) **t93**(件表示ボタンの色) **t94**(出所のジャンル名を全部出す) **t95**(選び直した直後の行の印)
...
- **行の背景は「買付」の3段階で決まる（v0.94.0）。** `pickTone(p)` が `"mid" | "done" | "ng"` を返し、
  クラスは `TONE_CLS`（白＝クラス無し / `pk-ok`＝青 / `pk-ng`＝赤）。追加した商品・チェックした商品の両方に付ける。
  どれになるかは **買付の選択肢に付けた色**（`TONE_OF_COLOR`）: 赤・ピンク＝`ng`、グレー＝`mid`、それ以外＝`done`。
  選択肢の値ではなく色で決めているのは、ユーザーが項目管理で文言も数も変えるため。
  値が今の選択肢に無いときは `stButton()` と同じく先頭にフォールバックする（**表示と判定を必ず揃える**）。
  - v0.93.0以前は「赤系の選択肢が1つでも付いたら赤」（`pickAlert`。楽天ライバル状況なども見ていた）＋
    「買付が済んだ行は薄く」（`wk-done`）だった。ユーザーの指定で**買付だけ**の3段階にして、薄くするのは廃止。
    `pickAlert` / `pickDone` / `.wk-done` は消してある。
...
- **0件と「全部まだ途中」は白1枚。** 青は「買付済がある」の意味になったので、
  昔の「全部ふつうなら既定の青」は廃止した（意味が衝突する）。
- 内訳（`title`）は買付の選択肢の**文言をそのまま**使う（`mix[].labels`）。ユーザーが名前を変えても付いてくる。
...
- **v0.94.0 行の色を買付の3段階に（途中=白 / 最後まで=青 / だめ=赤）。薄くするのは廃止（最新）**
