...
最終更新: 2026-08-25 / 現行版 **v0.53.0**

次のチャットに渡すもの:

1. `katabankakuninv0.53.0full.zip`（全ファイル入り。これが唯一の正）
...
**列の見た目（項目名・幅・揃え）は `data.cols` に入っていて products.json 経由で全端末に同期する**。
...
グループ内で最初に見つかった `label` を返す。項目編集で名前を入れたときは `colGroup(key)` の
全列に同じ値を書く。読み込み時は `unifyStCols()` が、グループ内で名前が食い違っている
（片方だけ付いている場合も含む）ものを既定へ戻す。`stTitle()` も `colLabelOf()` 経由。
...
- **`html{overflow-y:scroll;scrollbar-gutter:stable}`（v0.53.0）。** 縦スクロールバーの幅を常に
  確保する。行が増減するたびにスクロールバーが出入りすると表の幅が15pxほど動き、商品追加の
  直後に列がずれて見えた。ここを `auto` に戻すと再発する。
...
- **v0.53.0 状態列の項目名を表どうしで共通化 / 縦スクロールバー常時表示（最新）**
