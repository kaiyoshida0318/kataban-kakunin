...
最終更新: 2026-08-26 / 現行版 **v0.68.0**

次のチャットに渡すもの:

1. `katabankakuninv0.68.0full.zip`（全ファイル入り。これが唯一の正）
...
- **タブごとの差はSECTIONSのキーで表す**（v0.65.0）。列管理の設定は3つの巡回タブで共通なので、
  ...
  | `omit` | その列を `rankDefs()` から落とす（確認内容）。編集モーダルは `#fNote` を隠す |
...
- 出す場所は `urlCellHtml(obj, field, cut, attr)` に集約。右端の切り替えは `modeToggle()`。
  ハンドラは `[data-urlmode]`（一覧の行）と `[data-pickurlmode]`（商品）。
...
- **v0.68.0 ランキング2タブの「大カテゴリ」を「カテゴリー名」に（最新）**
